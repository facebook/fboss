...
- Represents the bidirectional wires connecting different hardware components on the board irrespective of the speed or any other configuration.
...
| --input-dir | Full directory path that has all vendor source files (default: `fboss/lib/platform_mapping_v2/platforms/PLATFORM_NAME/`). |
...
1. If not done already, add all platform mapping source files to `fboss/lib/platform_mapping_v2/platforms/PLATFORM_NAME/`.
2. If not done already, manually copy the generated JSON file into a C++ file at `agent/platforms/common/PLATFORM_NAME/PLATFORM_NAME.cpp`.
...
1. Create a new folder for your platform, `fboss/agent/platforms/common/PLATFORM_NAME/`, to host platform mapping C++ files that are baked into FBOSS binaries.
...
3. Add a source file `PLATFORM_NAMEPlatformMapping.cpp`, which contains the platform mapping JSON as a string, and a corresponding header file ([header example](https://github.com/facebook/fboss/blob/main/fboss/agent/platforms/common/montblanc/MontblancPlatformMapping.h), [source example](https://github.com/facebook/fboss/blob/main/fboss/agent/platforms/common/montblanc/MontblancPlatformMapping.cpp)).
4. Add a new switch case for your platform to `initPlatformMapping()` in `fboss/agent/platforms/common/PlatformMappingUtils.cpp` ([example](https://github.com/facebook/fboss/blob/main/fboss/agent/platforms/common/PlatformMappingUtils.cpp#L124)).
5. Create BspPlatformMapping header and source files under `fboss/lib/bsp/PLATFORM_NAME` ([header example](https://github.com/facebook/fboss/blob/main/fboss/lib/bsp/montblanc/MontblancBspPlatformMapping.h), [source example](https://github.com/facebook/fboss/blob/main/fboss/lib/bsp/montblanc/MontblancBspPlatformMapping.cpp)). Please follow instructions on generating the BspPlatformMapping JSON [here](https://github.com/facebook/fboss/blob/main/fboss/lib/bsp/bspmapping/README.md).
6. Add an LED Manager class for your platform as `fboss/led_service/PLATFORM_NAMELedManager.cpp` ([header example](https://github.com/facebook/fboss/blob/main/fboss/led_service/MontblancLedManager.h), [source example](https://github.com/facebook/fboss/blob/main/fboss/led_service/MontblancLedManager.cpp)).
7. Within `WedgeManagerInit.cpp`, add a function called `createPLATFORM_NAMEWedgeManager` function that instantiates a `WedgeManager` object with the platform mapping JSON file ([header example](https://github.com/facebook/fboss/blob/main/fboss/qsfp_service/platforms/wedge/WedgeManagerInit.h#L59), [source example](https://github.com/facebook/fboss/blob/main/fboss/qsfp_service/platforms/wedge/WedgeManagerInit.cpp#L181)).
8. Add a new `AgentPlatformsCommonPLATFORM_NAME.cmake` file for your platform ([example](https://github.com/facebook/fboss/blob/main/cmake/AgentPlatformsCommonMontblanc.cmake)). Then, add this library name to `sai_platform` link libraries [here](https://github.com/facebook/fboss/blob/main/cmake/AgentPlatformsSai.cmake#L104).
