...
class ConfigInterfaceFlowControlTest : public Fboss2IntegrationTest {
 protected:
  // Sets a flow-control attribute and records it for restoration in
  ...
  void setFlowControl(
    ...
    ASSERT_EQ(result.exitCode, 0) << "Failed to set " << attr << " to '"
                                  << value << "': " << result.stderr;
    commitConfig();
  }
  ...
  void setFlowControlRx(
      const std::string& interfaceName,
      const std::string& value) {
    setFlowControl(interfaceName, "flow-control-rx", value);
  }

  void setFlowControlTx(
      const std::string& interfaceName,
      const std::string& value) {
    setFlowControl(interfaceName, "flow-control-tx", value);
  }
  ...
  void TearDown() override {
    if (!touched_.empty()) {
      ...
      for (const auto& [interfaceName, attr] : touched_) {
        auto result =
            runCli({"config", "interface", interfaceName, attr, "disable"});
        if (result.exitCode != 0) {
          XLOG(WARN) << "TearDown failed to restore " << attr << " on "
 ...
 private:
  // (interfaceName, attr) pairs set by this test, restored in TearDown
