...
/**
 * End-to-end test for 'fboss2 --filter ... show route'.
 *
 ...
 * 3. Verify the routes appear under the expected --filter views (protocol,
 *    addressFamily, intersections), that the connected route is only visible
 *    under protocol == connected, and that invalid filter values are
...
TEST_F(ShowRouteFilterTest, FilterRoutes) {
  ...
  // protocol == static shows the injected routes (with the resolved nexthop
  // for the nexthop route); the connected route must not leak in. These hold
  // as long as no higher-priority client also owns the injected prefixes.
  EXPECT_TRUE(routeExists(
      {"--filter", "protocol == static", "show", "route"},
      kV4Nexthop,
      kV4NexthopAddr));
  EXPECT_TRUE(routeExists(
      {"--filter", "protocol == static", "show", "route"}, kV4Null0));
  EXPECT_TRUE(
      routeExists({"--filter", "protocol == static", "show", "route"}, kV4Cpu));
  EXPECT_TRUE(routeExists(
      {"--filter", "protocol == static", "show", "route"},
      kV6Nexthop,
      kV6NexthopAddr));
  EXPECT_FALSE(routeExists(
      {"--filter", "protocol == static", "show", "route"}, kV4ConnectedSubnet))
      << "Connected route leaked into protocol == static filter";

  // The connected route is visible under protocol == connected.
  EXPECT_TRUE(routeExists(
      {"--filter", "protocol == connected", "show", "route"},
      kV4ConnectedSubnet));
  EXPECT_FALSE(routeExists(
      {"--filter", "protocol == connected", "show", "route"}, kV4Nexthop));
  ...
  // Intersection filter: static && ipv6.
  EXPECT_TRUE(routeExists(
      {"--filter",
       "protocol == static && addressFamily == ipv6",
       "show",
       "route"},
      kV6Nexthop));
  EXPECT_FALSE(routeExists(
