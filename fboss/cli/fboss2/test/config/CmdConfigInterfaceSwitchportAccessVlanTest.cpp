...
TEST_F(
    CmdConfigInterfaceSwitchportAccessVlanTestFixture,
    queryClientThrowsWhenVlanHasNoInterface) {
  auto cmd = CmdConfigInterfaceSwitchportAccessVlan();
  utils::InterfaceList interfaces({"eth1/1/1"});
  ...
  try {
    ...
    std::string msg = e.what();
    EXPECT_THAT(msg, HasSubstr("VLAN 3000"));
    EXPECT_THAT(msg, HasSubstr("no interface"));
  }
  ...
  for (const auto& vlanPort : *config.sw()->vlanPorts()) {
    if (*vlanPort.logicalPort() == 1) {
      EXPECT_EQ(*vlanPort.vlanID(), 1);
    }
  }
}

// The default VLAN (sw.defaultVlan) is a valid target even when it has no
// L3 interface — FBOSS allows ports to be assigned to it without one.
TEST_F(
    CmdConfigInterfaceSwitchportAccessVlanTestFixture,
    queryClientSucceedsForDefaultVlanWithNoInterface) {
  auto cmd = CmdConfigInterfaceSwitchportAccessVlan();
  utils::InterfaceList interfaces({"eth1/1/1"});
  VlanIdValue vlanId(
      {"4000"}); // defaultVlan in fixture, no sw.interfaces entry

  auto result = cmd.queryClient(localhost(), interfaces, vlanId);
  EXPECT_THAT(result, HasSubstr("Successfully set access VLAN"));
  EXPECT_THAT(result, HasSubstr("4000"));

  auto& config = ConfigSession::getInstance().getAgentConfig();
  for (const auto& port : *config.sw()->ports()) {
    if (*port.name() == "eth1/1/1") {
      EXPECT_EQ(*port.ingressVlan(), 4000);
    }
  }
}
