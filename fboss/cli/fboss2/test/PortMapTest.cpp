...
class PortMapTest : public ::testing::Test {
 protected:
  cfg::AgentConfig loadConfigFromFile(const std::string& filename) {
    std::string configJson;
    if (!folly::readFile(filename.c_str(), configJson)) {
      throw std::runtime_error("Failed to read config file: " + filename);
    }
...
TEST_F(PortMapTest, Minipack3nPortIdMapping) {
  auto config = loadConfigFromFile(
      "/var/FBOSS/fboss/fboss/oss/link_test_configs/minipack3n.materialized_JSON");
...
TEST_F(PortMapTest, MontblancVlanMapping) {
  auto config = loadConfigFromFile(
      "/var/FBOSS/fboss/fboss/oss/link_test_configs/montblanc.materialized_JSON");
  PortMap portMap(config);
  ...
  portName = portMap.getPortNameForInterface(2003);
  ASSERT_TRUE(portName.has_value());
...
TEST_F(PortMapTest, PortExistenceChecks) {
  ...
  // Test hasInterface
  EXPECT_TRUE(portMap.hasInterface(2001));
  EXPECT_TRUE(portMap.hasInterface(2002));
  EXPECT_FALSE(portMap.hasInterface(9999));
}
...
TEST_F(PortMapTest, NonExistentLookups) {
  ...
  auto portName = portMap.getPortNameForInterface(9999);
...
TEST_F(PortMapTest, MontblancMultiplePorts) {
  ...
  std::vector<std::pair<std::string, int32_t>> expectedMappings = {
      {"eth1/1/1", 2001},
      {"eth1/2/1", 2003},
      {"eth1/2/5", 2004},
  };
...
TEST_F(PortMapTest, InterfacesWithoutPortMapping) {
  ...
  // mapping
  auto portName = portMap.getPortNameForInterface(10);
...
TEST_P(PortMapAllConfigsTest, CanLoadConfig) {
  std::string configFile = GetParam();
  ...
  auto portNames = portMap.getAllPortNames();
  EXPECT_GT(portNames.size(), 0)
      << "Config " << configFile << " should have at least one port";
  ...
  for (const auto& portName : portNames) {
    ...
    EXPECT_NE(port, nullptr) << "Port " << portName << " in config "
                             << configFile << " should be retrievable";
  }
  ...
  for (const auto& interfaceId : interfaceIds) {
    ...
    EXPECT_NE(interface, nullptr)
        << "Interface " << interfaceId << " in config " << configFile
