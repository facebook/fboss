...
TEST_F(CmdConfigSessionDiffTestFixture, diffSessionVsRevision) {
  ...
  // Should show a diff between r1 and session
  EXPECT_NE(result.find("DISABLED"), std::string::npos);
...
TEST_F(CmdConfigSessionDiffTestFixture, diffWithCurrentKeyword) {
  ...
  // Should show a diff between r1 and current system config
  EXPECT_NE(result.find("DISABLED"), std::string::npos);
  EXPECT_NE(result.find("ENABLED"), std::string::npos);
  EXPECT_NE(result.find("HUNDREDG"), std::string::npos);
}
