...
bool ConfigSession::hasActiveSession() const {
  ...
  // with a recorded BGP_PP_RESTART action, but never touching agent.conf.
...
void ConfigSession::saveBgpConfig() {
  ...
  recordServiceAction(
      cli::ServiceType::BGP_PP, cli::ConfigActionLevel::BGP_PP_RESTART);
}
...
void ConfigSession::initializeSession() {
  ...
  // BGP_PP_RESTART action on the next (separate-process) CLI invocation,
...
ConfigSession::CommitResult ConfigSession::commit(const HostInfo& hostInfo) {
  ...
  // saveBgpConfig() records BGP_PP_RESTART unconditionally, so re-committing an
  ...
  if (requiredActions_.count(cli::ServiceType::BGP_PP) > 0 &&
      bgpSessionExists()) {
  ...
  // drop BGP_PP when the BGP config is unchanged, so an unchanged BGP commit
  // neither promotes the config nor restarts bgp_pp.
  auto actions = requiredActions_;
  if (!bgpConfigChanged) {
    actions.erase(cli::ServiceType::BGP_PP);
  }
  ...
  const bool stagingBgp = actions.count(cli::ServiceType::BGP_PP) > 0;
