...
void atomicSymlinkUpdate(
  ...
  std::string tmpLinkName =
      "fboss2_tmp_" + boost::filesystem::unique_path().string();
  ...
  if (ec) {
    throw std::runtime_error(
        "Failed to create temporary symlink " + tempSymlinkPath.string() +
        " to " + newTarget + ": " + ec.message());
  }
...
void ConfigSession::copySystemConfigToSession() {
  ...
  if (!folly::readFile(sourceConfig.c_str(), configData)) {
    throw std::runtime_error("Failed to read config from " + sourceConfig);
  }
