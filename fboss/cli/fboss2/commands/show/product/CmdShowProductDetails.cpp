...
CmdShowProductDetails::RetType CmdShowProductDetails::queryClient(
...
CmdShowProductDetails::RetType CmdShowProductDetails::createModel(
  ...
  model.productVersion() =
      folly::to<std::string>(folly::copy(productInfo.productVersion().value()));
...
void CmdShowProductDetails::printOutput(
    const RetType& model,
    std::ostream& out) {
  ...
  out << "System Assembly Part Number: " << model.systemPartNumber().value()
      << std::endl;
  out << "System Manufacturing Date: " << model.mfgDate().value() << std::endl;
