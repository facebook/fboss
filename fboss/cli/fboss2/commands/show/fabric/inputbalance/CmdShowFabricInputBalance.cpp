...
CmdShowFabricInputBalance::RetType CmdShowFabricInputBalance::queryClient(
  ...
  if (!isDualStage) {
    ...
    auto dstClusterID = apache::thrift::can_throw(
        *nameToDsfNode.at(dstSwitchName.at(0)).clusterId());
    if (dsfNode.fabricLevel() == 2) {
      ...
      if (localClusterID.value() != dstClusterID) {
        ...
        auto neighborReachability =
            getNeighborReachability(neighbors, neighborToPorts, dstSwitchName);
...
CmdShowFabricInputBalance::RetType CmdShowFabricInputBalance::createModel(
...
void CmdShowFabricInputBalance::printOutput(
    const RetType& model,
    std::ostream& out) {
  utils::Table table;
  table.setHeader({
...
CmdShowFabricInputBalance::getNeighborReachability(
