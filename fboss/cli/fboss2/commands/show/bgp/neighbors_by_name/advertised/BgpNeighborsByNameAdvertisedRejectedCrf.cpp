...
BgpNeighborsByNameAdvertisedRejectedCrf::RetType
BgpNeighborsByNameAdvertisedRejectedCrf::queryClient(
  ...
  if (regexPatterns.empty()) {
    std::cout
        ...
        << "fboss2 show bgp neighbors-by-name <REGEX_PATTERN> advertised rejected crf"
        << std::endl;
    return {};
  }
  return BgpNeighborsByNameAdvertisedRejected::queryRejectedPrefixes(
      hostInfo, regexPatterns[0], prefixes, /*crfOnly=*/true);
}

void BgpNeighborsByNameAdvertisedRejectedCrf::printOutput(
...
template void CmdHandler<
    ...
    BgpNeighborsByNameAdvertisedRejectedCrfTraits>::run();
