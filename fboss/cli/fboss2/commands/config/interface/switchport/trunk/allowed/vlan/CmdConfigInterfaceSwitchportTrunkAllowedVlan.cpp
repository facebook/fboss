...
CmdConfigInterfaceSwitchportTrunkAllowedVlan::queryClient(
  ...
  // Get port logical IDs from port names. Fail the whole command if any
  // interface doesn't resolve to a port rather than silently applying a
  // partial change.
  ...
  for (const utils::Intf& intf : interfaces) {
    ...
    if (port) {
      portIds.push_back(*port->logicalID());
    } else {
  ...
  if (isAdd) {
    ...
    for (int32_t portLogicalId : portIds) {
      for (int32_t vlanId : vlanIds) {
  ...
  // Skip the save (and the commit history entry it records) when nothing
  // changed.
  if (changeCount == 0) {
    return fmt::format(
