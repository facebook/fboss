...
// The `acl` parent node dispatches to `table-group` and `table`. It needs a
// handler (rather than being a pure branch) so that addCommandBranch()
// increments depth before descending into the leaves — without that, both
 ...
 public:
  ...
  RetType queryClient(const HostInfo& /* hostInfo */) {
    throw std::runtime_error(
        "Incomplete command, please use 'table-group' or 'table' subcommand");
  }
