...
// unlike the sibling policy families (which parse `entry <seq-num>` inside
// the parent's ObjectArgType), the parent's parsed args arrive through the
// ancestor-args tuple, mirroring the `config interface ... switchport` chain.
