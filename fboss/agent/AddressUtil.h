...
inline thrift::Address toAddress(const folly::IPAddress& ip) {
  return ip.isV4()
    ? toAddressImpl(ip.asV4())
    : ip.isV6()
      ? toAddressImpl(ip.asV6())
      : thrift::Address();
}
...
thrift::BinaryAddress toBinaryAddressImpl(const IPAddressVx& addr) {
  ...
  result.addr.append(reinterpret_cast<const char *>(addr.bytes()),
                     IPAddressVx::byteCount());
  return result;
}

inline thrift::BinaryAddress toBinaryAddress(const folly::IPAddress& addr) {
  return addr.isV4()
    ? toBinaryAddressImpl(addr.asV4())
    : addr.isV6()
      ? toBinaryAddressImpl(addr.asV6())
...
} /* namespace network */ } /* namespace facebook */
