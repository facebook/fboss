...
const HwAsic* checkSameAndGetAsic(
  ...
  // In multi-NPU setups, callers pass switchId to select the ASIC for a
  // specific NPU, getting switch-scoped properties (e.g. switchType,
  // maxLagMemberSize) for the correct NPU.
  if (switchId) {
    auto targetSwitchId = SwitchID(switchId);
    ...
    throw FbossError("No asic found with switchId: ", switchId);
  }
  return *asics.begin();
}
