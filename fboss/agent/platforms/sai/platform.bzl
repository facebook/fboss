...
load("//fboss/agent/hw/sai/impl:impl.bzl", "get_all_npu_impls", "get_all_phy_impls", "get_ld_library_path", "get_link_group_map", "to_impl_lib_name", "to_impl_suffix", "to_versions")
load("//fboss/agent/hw/sai/switch:switch.bzl", "sai_switch_dependent_name", "sai_switch_lib_name")
load("//fboss/build:build_info.bzl", "gen_sdk_build_info_src")

headers = [
    "SaiBcmBlackwolf800banwPlatformPort.h",
    "SaiBcmDarwinPlatformPort.h",
    "SaiBcmElbertPlatformPort.h",
    "SaiBcmFujiPlatformPort.h",
    "SaiBcmIcecube800PlatformPort.h",
    "SaiBcmIcetea800bcPlatformPort.h",
    "SaiBcmJ4SimPlatformPort.h",
    "SaiBcmLadakh800bclsPlatformPort.h",
    "SaiBcmMinipack3BTAPlatformPort.h",
    "SaiBcmMinipackPlatformPort.h",
    "SaiBcmMontblancPlatformPort.h",
    "SaiBcmTahansb800bcPlatformPort.h",
    "SaiBcmWedge100PlatformPort.h",
    "SaiBcmWedge800BACTPlatformPort.h",
    "SaiWedge800CACTPlatformPort.h",
...
common_srcs = [
    ...
    "SaiBcmBlackwolf800banwPlatform.cpp",
    "SaiBcmBlackwolf800banwPlatformPort.cpp",
    ...
    "SaiBcmMinipack3BTAPlatform.cpp",
    "SaiBcmMinipack3BTAPlatformPort.cpp",
    ...
    "SaiBcmIcecube800bcPlatform.cpp",
    "SaiBcmIcecube800PlatformPort.cpp",
    "SaiBcmIcecube800banwPlatform.cpp",
    "SaiBcmIcetea800bcPlatform.cpp",
    "SaiBcmIcetea800bcPlatformPort.cpp",
    "SaiBcmJ4SimPlatform.cpp",
    "SaiBcmJ4SimPlatformPort.cpp",
    "SaiBcmLadakh800bclsPlatform.cpp",
    "SaiBcmTahansb800bcPlatform.cpp",
    "SaiBcmWedge800BACTPlatform.cpp",
    "SaiBcmWedge800BACTPlatformPort.cpp",
    "SaiWedge800CACTPlatform.cpp",
    "SaiWedge800CACTPlatformPort.cpp",
    ...
    "SaiYangraPlatform.cpp",
    "SaiMinipack3NPlatform.cpp",
    "SaiYangraPlatformPort.cpp",
    "SaiMinipack3NPlatformPort.cpp",
    "SaiYangra2Platform.cpp",
    "SaiYangra2PlatformPort.cpp",
]
...
brcm_pai_srcs = common_srcs + [
    "SaiPhyPlatform.cpp",
    # TODO(T244670564) Broadcom PAI Platform shouldn't care the following SaiPlatforms
    "oss/SaiBcmMinipackPlatform.cpp",
    "oss/SaiBcmPlatform.cpp",
    "oss/SaiBcmMinipackPlatformPort.cpp",
    "oss/SaiBcmFujiPlatformPort.cpp",
...
def _get_srcs(sai_impl):
    ...
    if sai_impl.name == "credo":
        return phy_srcs
    if sai_impl.name == "nvda":
        # TODO(pshaikh): fix this
        return fake_srcs
    if sai_impl.name == "brcm_pai":
        return brcm_pai_srcs
    return fake_srcs
...
def _select_link_group_map_for_opt(binary_name, sai_impl):
    """
    Returns a select() that chooses the appropriate link_group_map based on build mode.
    Only asan modes (sanitizers - opt-asan, opt-tsan ) uses is_prod=False (use eviction map).
    All other modes (dev, dbg, opt etc) use is_prod=True (no eviction map).
    """

    # Get link group maps for both modes
    link_group_map_prod = get_link_group_map(binary_name, sai_impl, is_prod = True)
    link_group_map_asan = get_link_group_map(binary_name, sai_impl, is_prod = False)

    # Restrict special handling to opt-asan and opt-tsan only.
    # All other modes (dev, dbg, plain opt) use the prod map.
    return select({
        "DEFAULT": link_group_map_prod,
        # opt base (no sanitizer) -> dev map (no special copy/eviction handling)
        "ovr_config//build_mode:opt": select({
            "DEFAULT": link_group_map_prod,
            # Only when sanitizer is asan or tsan, use the prod (eviction-free) map
            "ovr_config//build_mode:sanitizer_type[asan]": link_group_map_asan,
            "ovr_config//build_mode:sanitizer_type[tsan]": link_group_map_asan,
        }),
    })

def _wedge_agent_bin(sai_impl, is_npu, is_test):
    wedge_agent_name = "wedge_agent-test{}".format(to_impl_suffix(sai_impl)) if is_test else "wedge_agent{}".format(to_impl_suffix(sai_impl))

    build_info_name = "{}-sdk-build-info".format(wedge_agent_name)
    gen_sdk_build_info_src(name = build_info_name, sdk = sai_impl)
    ...
    return cpp_binary(
        ...
        srcs = [
            "wedge_agent.cpp",
            ":{}".format(build_info_name),
        ],
        auto_headers = AutoHeaders.SOURCES,
        link_group_map = get_link_group_map(wedge_agent_name, sai_impl, is_prod = True),
        linker_flags = [
            "--export-dynamic",
        ...
        deps = [
            ...
            ("//fboss/agent:setup_thrift_test" if is_test else "//fboss/agent:setup_thrift_prod"),
        ],
    )

def get_version_info_test_env(sai_impl):
    test_env = {}
    ld_library_path = get_ld_library_path(sai_impl)
    if not ld_library_path:
        return test_env

    test_env["LD_LIBRARY_PATH"] = "{}".format(ld_library_path)
    return test_env
...
def _wedge_hwagent_bin(sai_impl, hwagent_prefix):
    ...
    if sai_impl.name == "fake" or sai_impl.name == "leaba" or sai_impl.name == "nvda":
    ...
    cpp_binary(
        ...
        auto_headers = AutoHeaders.SOURCES,
        link_group_map = _select_link_group_map_for_opt(wedge_agent_name, sai_impl),
        linker_flags = [
            "--export-dynamic",
            "--unresolved-symbols=ignore-all",
        ],
...
def all_wedge_agent_bins():
    for sai_impl in get_all_npu_impls():
        ...
        _wedge_agent_bin(sai_impl, is_npu = True, is_test = False)

        # This test binary is identical to wedge_agent except that it links with
        # setup_thrift_test library. This library supports an extended
        # set of Thrift endpoints that can perform disruptive operations like
        # coldboot and thus are applicable and safe for test frameworks only.
        #
        # Today: SAI Multi Node Tests are supported only for DNX.
        # TODO: Extend to support other ASICs/platforms.
        if "dnx" in sai_impl.version:
            _wedge_agent_bin(sai_impl, is_npu = True, is_test = True)
