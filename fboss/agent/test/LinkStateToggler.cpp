...
bool LinkStateToggler::waitForPortEventsOrSkipIfAlreadyInState(
  ...
  if (!FLAGS_multi_switch) {
    // Check if all ports are already in the desired OperState
    bool allPortsAlreadyInDesiredState = true;
    ...
    for (const auto& portId : ports) {
      ...
      if (port->getOperState() != desiredOperState) {
        allPortsAlreadyInDesiredState = false;
        break;
      }
    }

    if (allPortsAlreadyInDesiredState) {
      // All ports are already in the desired state, skip waiting for events
      XLOG(DBG2) << "All ports' OperState already in desired "
                 << (up ? "UP" : "DOWN")
                 << " state, skipping event wait and state update";
      std::lock_guard<std::mutex> lock{linkEventMutex_};
      // Clear the waiting state since we're skipping the wait
      portIdToWaitFor_.clear();
      desiredPortEventsOccurred_ = true;
    } else {
      // At least one port needs state change, wait for events
