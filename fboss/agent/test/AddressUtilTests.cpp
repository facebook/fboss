...
TEST(AddressUtilTests, ToBinaryAddressV4) {
  ...
  EXPECT_FALSE(thriftBinary.port());
  EXPECT_FALSE(thriftBinary.ifName());
}
...
TEST(AddressUtilTests, toIpPrefixV6) {
  ...
  EXPECT_EQ(kV6Ip, toIPAddress(*ipPfx.ip()));
  EXPECT_EQ(64, ipPfx.prefixLength());
}

TEST(AddressUtilTests, toIpPrefixV4) {
  auto ipPfx = facebook::fboss::toIpPrefix(folly::CIDRNetwork{kV4Ip, 24});
  EXPECT_EQ(kV4Ip, toIPAddress(*ipPfx.ip()));
  EXPECT_EQ(24, ipPfx.prefixLength());
}
