...
class AgentEgressForwardingDiscardsCounterTest : public AgentHwTest {
 public:
  cfg::SwitchConfig initialConfig(
    ...
    auto firstPortId = ensemble.masterLogicalInterfaceOrHyperPortIds()[0];
...
TEST_F(AgentEgressForwardingDiscardsCounterTest, outForwardingDiscards) {
  auto egressPortDesc =
      PortDescriptor(masterLogicalInterfaceOrHyperPortIds()[0]);
  auto setup = [=, this]() {
  ...
  auto verify = [=, this]() {
    ...
    getSw()->sendPacketOutOfPortAsync(
        std::move(pkt), masterLogicalInterfaceOrHyperPortIds()[1]);
    WITH_RETRIES({
