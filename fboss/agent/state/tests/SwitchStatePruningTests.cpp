...
void addNeighborEntry(
  ...
  auto oldNeighborTable =
      (*state)->getVlans()->getVlan(*vlan)->template getNeighborTable<NTable>();
  if (oldNeighborTable) {
  ...
  if (!oldNeighborTable) {
    ...
    auto vlanPtr = (*state)->getVlans()->getVlan(*vlan).get();
...
TEST(SwitchStatePruningTests, AddNeighborEntry) {
  ...
  auto h3entry =
      state3->getVlans()->getVlan(host3vlan)->getArpTable()->getEntry(host3ip);
  ...
  auto v6entry =
      state4->getVlans()->getVlan(host2vlan)->getNdpTable()->getEntry(host2ip);
...
TEST(SwitchStatePruningTests, ChangeNeighborEntry) {
  ...
  auto entry1old =
      state2->getVlans()->getVlan(host1vlan)->getArpTable()->getEntry(host1ip);
  auto entry2old =
      state2->getVlans()->getVlan(host2vlan)->getArpTable()->getEntry(host2ip);
  auto entry3old =
      state2->getVlans()->getVlan(host3vlan)->getNdpTable()->getEntry(host3ip);
  ...
  auto entry1new =
      state3->getVlans()->getVlan(host1vlan)->getArpTable()->getEntry(host1ip);
  auto entry2new =
      state3->getVlans()->getVlan(host2vlan)->getArpTable()->getEntry(host2ip);
  auto entry3new =
      state3->getVlans()->getVlan(host3vlan)->getNdpTable()->getEntry(host3ip);
...
TEST(SwitchStatePruningTests, ModifyArpTableMultipleTimes) {
  ...
  ASSERT_NE(
      state1->getVlans()->getVlan(host1vlan),
      state2->getVlans()->getVlan(host1vlan));
  ASSERT_NE(
  ...
  auto arpTable22Ptr =
      state2->getVlans()->getVlan(host4vlan)->getArpTable()->modify(
          host4vlan, &state3);
