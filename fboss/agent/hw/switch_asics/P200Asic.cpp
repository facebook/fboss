...
bool P200Asic::isSupportedNonFabric(Feature feature) const {
  switch (feature) {
    ...
    case HwAsic::Feature::SAI_PRBS:
      /*
       * Source port ACL qualifier is used in certain HW tests to trap packets
       * to CPU. P200 always runs a P4-warmboot-capable SDK, which uses the
       * source port qualifier, so this feature is supported.
       */
      return true;
  }
  return false;
...
bool P200Asic::isSupportedFabric(Feature feature) const {
  switch (feature) {
    case HwAsic::Feature::REMOVE_PORTS_FOR_COLDBOOT:
      return true;
    default:
...
int P200Asic::getDefaultNumPortQueues(
...
std::set<cfg::StreamType> P200Asic::getQueueStreamTypes(
    cfg::PortType portType) const {
  switch (portType) {
    ...
    case cfg::PortType::HYPER_PORT_MEMBER:
      ...
      // recycle port for P200 ASIC
      break;
  }
  throw FbossError(
      "P200 ASIC does not support:",
      apache::thrift::util::enumNameSafe(portType));
}
cfg::Range64 P200Asic::getReservedEncapIndexRange() const {
...
P200Asic::desiredLoopbackModes() const {
  ...
  throw FbossError("Unsupported switchType for P200: ", getSwitchType());
}
} // namespace facebook::fboss
