 ...
 *
 * This will use a SaiHwSwitch for the HwSwitch implementation, with the
 * specified initial state.  The returned SwSwitch will have already been
 * initialized.
 */
std::unique_ptr<SwSwitch> createSaiSw(const std::shared_ptr<SwitchState> &);
std::unique_ptr<SwSwitch> createSaiSw(const std::shared_ptr<SwitchState> &,
                                      const folly::MacAddress &);

std::unique_ptr<SwSwitch> createSaiSw(cfg::SwitchConfig *cfg,
...
/*
 * Get the SaiSwitch from a SwSwitch.
 */
SaiSwitch *getSaiHw(SwSwitch *sw);

/*
 * Get the SaiPlatform from a SwSwitch.
 */
...
/*
 * Convenience macro that wraps EXPECT_CALL() on the underlying Saiwitch
 */
#define EXPECT_HW_CALL(sw, method) \
  EXPECT_CALL(*getSaiHw((sw).get()), method)

/*
 * Convenience macro that wraps EXPECT_CALL() on the underlying SaiPlatform
 */
#define EXPECT_PLATFORM_CALL(sw, method) \
  EXPECT_CALL(*getSaiPlatform((sw).get()), method)
