...
unique_ptr<SwSwitch> createSaiSw(const shared_ptr<SwitchState> &state) {
  auto sw = make_unique<SwSwitch>(make_unique<SaiPlatform>());
...
unique_ptr<SwSwitch> createSaiSw(const shared_ptr<SwitchState> &state,
  ...
  auto platform = make_unique<SaiPlatform>();
...
unique_ptr<SwSwitch> createSaiSw(cfg::SwitchConfig *config,
                                 MacAddress mac,
                                 uint32_t maxPort) {
...
unique_ptr<SwSwitch> createSaiSw(cfg::SwitchConfig *config,
                                 uint32_t maxPort) {
  return createSaiSw(config, MacAddress("02:00:00:00:00:01"), maxPort);
}

SaiSwitch *getSaiHw(SwSwitch *sw) {
  return boost::polymorphic_downcast<SaiSwitch *>(sw->getHw());
}

SaiPlatform *getSaiPlatform(SwSwitch *sw) {
  return boost::polymorphic_downcast<SaiPlatform *>(sw->getPlatform());
}
...
/*  auto intf1 = make_shared<Interface>
               (InterfaceID(1), RouterID(0), VlanID(1),
                "interface1", MacAddress("00:02:00:00:00:01"), InterfaceID(5), PortID(1));
  ...
  auto intf55 = make_shared<Interface>
                (RouterID(0), VlanID(55),
                 "interface55", MacAddress("00:02:00:00:00:55"), InterfaceID(5), PortID(55));
