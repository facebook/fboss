...
TEST_F(Srv6ManagerTest, addSrv6SidList) {
  ...
  auto handle =
      saiManagerTable->srv6Manager().addOrReuseSrv6SidList(attrs, attrs);
...
TEST_F(Srv6ManagerTest, getSrv6SidListHandle) {
  ...
  auto holder =
      saiManagerTable->srv6Manager().addOrReuseSrv6SidList(attrs, attrs);
  auto* handle = saiManagerTable->srv6Manager().getSrv6SidListHandle(attrs);
  EXPECT_NE(handle, nullptr);
  EXPECT_NE(handle->sidList, nullptr);
}
...
TEST_F(Srv6ManagerTest, addDifferentSidLists) {
  ...
  auto handle1 =
      saiManagerTable->srv6Manager().addOrReuseSrv6SidList(attrs1, attrs1);
  auto handle2 =
      saiManagerTable->srv6Manager().addOrReuseSrv6SidList(attrs2, attrs2);
