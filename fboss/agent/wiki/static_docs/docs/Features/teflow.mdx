...
## FHTEv2 Solution
FHTEv2 Solution implemented to improve the scaling (to support 32K GPU jobs) and performance (sub optimal performance on non all to all jobs).
As part of the FHTEv2 solution to support the flow matrix to calculate traffic rate on each RTSW switch a new feature emStatOnlyMode implemented in FBOSS wedge agent. emStatOnlyMode will allocate hardware counters for inactive flows(no next hop EM entries). These counters are used by flow matrix to determine the traffic rate on each switch which in turn fed into the controller's CSPF algorithm for optimal routes.
...
TeFlow config are part of switch settings configuration of agent config. The exactMatchTableConfigs need to be set as below to enable TeFlow feature.
...
Use fboss2 show teflow command to display all the telfows information On BCM shell, fp show displays the hardware objects
...
```
<div>
Sphinx Source: [fbsource/fbcode:fboss/agent/wiki/teflow.rst](https://internalfb.com/intern/diffusion/FBS/browse/master/fbcode/fboss/agent/wiki/teflow.rst)</div>
