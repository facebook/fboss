...
```
--enable-replayer --enable_get_attr_log --enable_packet_log --sai-log [PATH TO LOG] --sai_replayer_sdk_log_level [DEBUG|INFO|NOTICE|WARN|ERROR|CRITICAL]
```

OSS `run_test.py` Script:
```
--sai_replayer_logging [PATH TO LOGS] --sai_replayer_sdk_log_level [DEBUG|INFO|NOTICE|WARN|ERROR|CRITICAL]
```
