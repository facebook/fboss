...
Once routes are resolved in the RIB, the FIB is updated through the `RibToSwitchStateUpdater` callback mechanism. This is the bridge between the RIB and SwitchState.

The `RibToSwitchStateUpdater`:
...
| RibToSwitchStateUpdater | `agent/rib/RibToSwitchStateUpdater.cpp` | RIB to FIB callback |
