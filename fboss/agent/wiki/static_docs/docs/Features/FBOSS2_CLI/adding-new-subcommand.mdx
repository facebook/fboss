...
Define a traits class deriving BaseCommandTraits and specify argument and return types.

**New commands (recommended):** To declare a positional argument, set `ObjectArgType` to the argument class and provide a `static void addCliArg(CLI::App& cmd, std::vector<std::string>& args)` method that calls `cmd.add_option(...)` with the positional name and help text. This approach keeps argument registration local to the command — no central enum or switch to touch. Use `argRegistrar<Traits>` in the command tree.

**Existing commands (legacy):** Many non-config commands still use the older `ObjectArgTypeId` enum approach, where the traits struct declares a `static constexpr utils::ObjectArgTypeId ObjectArgTypeId = ...` and the central switch in `CmdSubcommands.cpp` handles CLI11 registration. These commands use `argTypeHandler<Traits>` in the command tree. The FBOSS team is gradually migrating these to the new `addCliArg` pattern.
