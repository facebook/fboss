...
Additional options are available to customize the script's behavior:
```bash
./postProcess.py --help
```

For buck compilation, the flag `--generate_run_bzl` can be added which will also consume a `run.bzl` file from the same input directory. An example compilation command would be:
```bash
buck2 build --show-output @//mode/opt //fboss/agent/hw/sai/tracer/run/output:sai_replayer-brcm-10.2.0.0_odp
```
