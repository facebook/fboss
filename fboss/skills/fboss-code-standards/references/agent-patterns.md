...
### 19. Preserve the Split-Agent Dependency Boundary
**Check**: When changing dependencies reachable from a production HW-agent executable, inspect the configured production dependency graph and ensure it does not include SW-agent implementation libraries. Resolve missing headers, types, helpers, or generated interface definitions through the narrow library that owns them, or extract shared functionality into a focused library that is independent of the SW-agent implementation. This rule does not apply to SW-agent executables, monolithic agent executables, or explicitly test-only targets.
**Why**: Pulling SW-agent implementation libraries into the HW process violates the runtime boundary, increases link time and binary size, and makes unrelated SW-agent additions affect every HW-agent executable.
**Confidence**: HIGH (a new configured production HW-agent path is a layering regression)
