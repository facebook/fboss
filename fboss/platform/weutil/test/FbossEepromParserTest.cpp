...
TEST(FbossEepromParserTest, parserV4) {
  // Buffer dumped from a montblanc (V4) EEPROM.

  std::vector<uint8_t> eepromv4 = {
  ...
  std::vector<std::pair<std::string, std::string>> expectedContents = {
  ...
  folly::writeFile(eepromv4, fileName.c_str());
...
TEST(FbossEepromParserTest, parserV5) {
  ...
  folly::test::TemporaryDirectory tmpDir = folly::test::TemporaryDirectory();
  std::string fileName = tmpDir.path().string() + "/eepromContent";
  folly::writeFile(eepromv5, fileName.c_str());
  FbossEepromParser parser(fileName, 0);
  auto parsedContents = parser.getContents();
  ASSERT_EQ(expectedContents.size(), parsedContents.size());
