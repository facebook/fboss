...
WeutilDarwin::WeutilDarwin() {
  ...
  if (retVal) {
    throw std::runtime_error("Cannot create layout file with: " + kCreateLayout_);
  }
  ...
  if (retVal) {
    throw std::runtime_error("Cannot create BIOS file with: " + kFlashRom_ + " " + std::to_string(retVal));
  }
...
void WeutilDarwin::printInfo () {
  ...
  for (auto item : weFields_) {
    if (item.first == "Wedge EEPROM"){
      std::cout << item.first << folly::to<std::string>(" ", item.second, ":") << std::endl;
    }
    else {
      auto it = kMapping_.find (item.first);
      std::cout << folly::to<std::string>(item.first, ": ")
        << (it == kMapping_.end()? item.second : prefdl.getField(it->second))
        << std::endl;
    }
  }
