...
PsuHelper::PsuHelper(int busNum, std::string devAddr)
    : devAddr_(static_cast<uint8_t>(std::stoul(devAddr, nullptr, 16))),
      i2cPsuIo_(std::make_unique<I2cDevIo>(
          fmt::format("/dev/i2c-{}", busNum),
          I2cIoType::I2cIoTypeRdWr)) {}

void PsuHelper::dumpRegisters() const {
  constexpr std::string_view errMsg = "Error: failed to read register `{}`: {}";
  for (const auto& reg : PsuHelper::genericPsuRegisters_) {
    ...
    try {
      ...
      std::cout << fmt::format(errMsg, registerName, e.what()) << std::endl;
      continue;
    }
    ...
    if (registerName == "READ_VOUT") {
      try {
        ...
        std::cout << fmt::format(
                         errMsg, std::get<0>(voutModeRegister_), e.what())
                  << std::endl;
        continue;
      }
    }
    ...
    if (valType == ValueType::ASCII) {
      ...
      std::cout << fmt::format(errMsg, registerName, "unknown data format")
                << std::endl;
    }
  }
}
