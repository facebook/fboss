...
uint32_t mmap_read(uint32_t address, char acc_type) {
  ...
  map_base = mmap(nullptr, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, address & ~MAP_MASK);
  ...
  std::cout << "Value at address 0x" << std::hex << address << " (0x" << virt_addr << "): 0x" << read_result << std::endl;
...
int mmap_write(uint32_t address, char acc_type, uint32_t val) {
  ...
  std::cout << "Written 0x" << std::hex << val << "; readback 0x" << read_result << std::endl;
