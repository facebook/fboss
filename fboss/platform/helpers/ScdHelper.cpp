...
ScdHelper::ScdHelper() {
  ...
  sysfsPath = folly::to<std::string>("/sys/bus/pci/devices/0000:",
    busIdStr, "/resource0");
  ...
  // user space
  mapSize_ = 0x80000;
  mapAddr_ = (uint8_t *)mmap(nullptr, mapSize_,
...
void ScdHelper::kick_wdt(uint32_t seconds) {
  if (seconds > 655) {
    LOG(INFO) << "Timeout value can not be greater than 655s, set to maximum 655s";
    seconds = 655;
  }
...
void ScdHelper::writeReg(uint32_t regAddr, uint32_t val) {
  if (regAddr >= mapSize_-3) {
    throw std::runtime_error("Register offset to write exceeds memory map size");
    return;
  }

  LOG(INFO) << "Reg 0x:" << std::hex << regAddr << " original value is 0x"
    << *(volatile unsigned int*) (mapAddr_+regAddr) << ", will write 0x" << val;
  *(volatile unsigned int*) (mapAddr_+regAddr) = val;
  LOG(INFO) << "Reg 0x:" << std::hex << regAddr
    << " current value after updating is 0x" << *(volatile unsigned int*)(mapAddr_+regAddr);
}
