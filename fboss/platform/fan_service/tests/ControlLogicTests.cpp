...
TEST_F(ControlLogicTests, CalculatePid) {
  ...
  //         is too high. PWM should go down.
  PwmCalcCache cache1;
  cache1.previousTargetPwm = 60;
  cache1.previousRead1 = 49;

  auto newPwm =
      controlLogic_->calculatePid("qsfp_group_1", 50, cache1, pidSetting, 5);
  EXPECT_EQ(newPwm, 0);
  EXPECT_EQ(cache1.previousTargetPwm, 0);
  EXPECT_EQ(cache1.previousRead1, 50);
  EXPECT_EQ(cache1.previousRead2, 49);
  EXPECT_EQ(cache1.last_error, 8);

  // CASE 2: The read value is within desired range. PWM should not change.
  PwmCalcCache cache2;
  cache2.previousTargetPwm = 30;
  cache2.previousRead1 = 52;
  newPwm =
      controlLogic_->calculatePid("qsfp_group_1", 59, cache2, pidSetting, 5);
  EXPECT_EQ(newPwm, 30);
  EXPECT_EQ(cache2.previousTargetPwm, 30);
  EXPECT_EQ(cache2.previousRead1, 59);
  EXPECT_EQ(cache2.previousRead2, 52);
  EXPECT_EQ(cache2.last_error, 0);

  // CASE 3: The read value is higher than setPoint, and the current target PWM
  //         is too low. PWM should go up a bit.
  PwmCalcCache cache3;
  cache3.previousTargetPwm = 30;
  cache3.previousRead1 = 52;
  newPwm =
      controlLogic_->calculatePid("qsfp_group_1", 68, cache3, pidSetting, 5);
  EXPECT_EQ(newPwm, 24);
  EXPECT_EQ(cache3.previousTargetPwm, 24);
  EXPECT_EQ(cache3.previousRead1, 68);
  EXPECT_EQ(cache3.previousRead2, 52);
  EXPECT_EQ(cache3.last_error, -8);

  // CASE 4: Though read value is higher than setPoint, the current target PWM
  //         is overly high. PWM should go down.
  PwmCalcCache cache4;
  cache4.previousTargetPwm = 99;
  cache4.previousRead1 = 52;
  newPwm =
      controlLogic_->calculatePid("qsfp_group_1", 68, cache4, pidSetting, 5);
  EXPECT_EQ(newPwm, 24);
  EXPECT_EQ(cache4.previousTargetPwm, 24);
  EXPECT_EQ(cache4.previousRead1, 68);
  EXPECT_EQ(cache4.previousRead2, 52);
  EXPECT_EQ(cache4.last_error, -8);
}
} // namespace facebook::fboss::platform
