...
- service logs, generated configs, and result CSVs, routed into the same directory.
...
from fboss_test_runner.constants import GENERATED_CONFIG_ROOT
...
def collect_generated_configs(
    run_dir: str,
    test_type: str | None,
    since: float,
    source_root: str = GENERATED_CONFIG_ROOT,
) -> None:
    """Copy configs generated by this test run into its log bundle."""
    if test_type is None:
        return

    source_dir = os.path.join(source_root, test_type)
    destination_dir = os.path.join(run_dir, "generated_configs", test_type)
    for config_path in glob.glob(os.path.join(source_dir, "*")):
        try:
            if not os.path.isfile(config_path) or os.path.getmtime(config_path) < since:
                continue
            os.makedirs(destination_dir, exist_ok=True)
            shutil.copy2(config_path, destination_dir)
        except OSError as e:
            print(f"Could not collect generated config {config_path}: {e}")
...
class LogCapture:
    ...
    def stop(self) -> None:
        if self._tee is not None:
            self._tee.stop()
        # Pull this run's logs, generated configs, and results into the run dir
        ...
        collect_generated_configs(self._run_dir, self._test_type, self._run_start)
